...
const workerProgram = [
  ...
  "if(q.op==='warm')result={revision:0,plan:{constraints:[],responseStyle:[],continuity:[],topicActivated:[],deepRecall:[],doNotSurface:[]}};",
...
const extractionWorkerProgram = [
  ...
  "r.on('line',line=>{const q=JSON.parse(line);let result={};",
  "if(q.op==='health')result={protocolVersion:1,schemaVersion:3,predicateKeys:[],predicateSchemas:[]};",
  "if(q.op==='warm')result={revision:admitted,plan:{constraints:[],responseStyle:admitted?[{recordId:'admit-observed',text:'extraction_observed',surface:'freely_mentionable',reason:'test'}]:[],continuity:[],topicActivated:[],deepRecall:[],doNotSurface:[]}};",

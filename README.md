...
Current verification snapshot (2026-09-22): 334 of 335 normative requirements have usable evidence, with one evidence gap, [REQ-EVAL-035](docs/traceability-gap-register.json). The execution report records 45 spec files and 344 tests, with 333 passed and 11 skipped; the default no-credential run records 41 files, 329 passed and 15 skipped. Skipped cases are opt-in provider/data campaigns, not failures. This snapshot does not claim that the external LoCoMo or LongMemEval benchmarks are complete.
...
- External benchmark closure remains open: REQ-EVAL-035 requires real, resumable LoCoMo and LongMemEval campaign executions with their metric artifacts. An adapter experiment that produced empty recall context or a zero score is not accepted as evidence.

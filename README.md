...
| `recallRawEvidenceMaxCandidates` | `2` | Maximum raw L0 evidence candidates considered per recall. | Included. |
...
Current verification snapshot (2026-09-22): 334 of 335 normative requirements have usable evidence, with one evidence gap, [REQ-EVAL-035](docs/traceability-gap-register.json). The execution report records 47 spec files and 355 tests, with 344 passed and 11 skipped; the default no-credential run records 43 files, 340 passed and 15 skipped. Skipped cases are opt-in provider/data campaigns, not failures. A real recall-only smoke observed 9/12 non-empty LoCoMo queries and 2/2 non-empty LongMemEval-S queries; this is not the full answer/scorer campaign and does not claim either external benchmark complete.
...
- External benchmark closure remains open: REQ-EVAL-035 requires real, resumable LoCoMo and LongMemEval campaign executions with their metric artifacts. The first recall-only smoke reached non-systematic recall for both datasets, but the full answer/scorer campaign is still pending; an adapter experiment that produced empty recall context or a zero score is not accepted as evidence.

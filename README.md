...
The `candidateAutoConfirm` policy can admit an extracted page without a manual confirmation step, and it defaults to `user_grounded`. A candidate is admitted only when its description exactly matches the normalized full text of one user-authored L0 event in the same scope. The canonical title, description and body are then all replaced with that exact user text, preserving negations and qualifications and discarding any model-authored additions or omissions. Dream-authored tags, categories, page types and temporal claims are also discarded when the canonical page is created. Candidate sensitivity is normalized locally from title, description and body: private claims become `sensitive`, identifier-shaped claims become `provisional_sensitive`, and only `normal` candidates can be admitted; a malformed sensitivity label fails closed as provisional. Conflicting candidates stay pending. `off` keeps every candidate pending. `all` skips grounding and is a deliberate deviation from the no-auto-promotion invariant, intended only for controlled dogfooding. Every automatic admission is audited as `candidate-auto-confirmed` with its mode and grounding reference.
...
| `dreamApiUrl` | `https://generativelanguage.googleapis.com/v1beta` | Google Gemini/Gemma native `generateContent` base used for Wiki extraction. | Included through the persisted Dream settings. |
| `dreamCredentialRef` | `GEMINI_API_KEY` | Credential reference resolved when Dream calls the provider. | Included; secret value is never returned. |
| `dreamModel` | `gemini-3.5-flash-lite` | Model name sent to the Dream provider. | Included through the persisted Dream settings. |
| `dreamFallbacks` | `[]` in the core plugin; the Web composition currently supplies the qualified `gemma-4-26b-a4b-it` | Up to two ordered providers tried only after a retryable Dream failure; each gets one request with a 60-second timeout. Models without a 3/3 real acceptance stay out of the formal default list. | Not returned; credential references and provider URLs stay deployment configuration. |
...
| `candidateAutoConfirm` | `user_grounded` | Candidate auto-confirmation policy. The default requires a description grounded by the full normalized user event; all canonical text fields are then replaced by the original user text, and locally classified `normal` sensitivity is required. `off` keeps candidates pending and `all` skips grounding. | Included. |
...
Fallbacks are tried in order after network/timeout, HTTP 429/5xx, or invalid FILE output. HTTP 400/401/403 stops the chain; a valid response with pending candidates is still a completed Dream. Configure each `credentialRef` in the host credential store. Do not save a key through the UI, write it to `dream-settings.json`, commit it, put it in a URL or paste it into logs. Production acceptance uses Google's native Gemini/Gemma `generateContent` protocol; OpenAI-compatible and Anthropic-compatible adapters remain available only for explicit operator configurations.
...
Current verification snapshot (2026-09-24): 334 of 335 normative requirements have usable evidence, with one evidence gap, [REQ-EVAL-035](docs/traceability-gap-register.json). The normalized execution report records 47 spec files and 422 tests, with 420 passed, 0 failed and 2 refresh-only skipped checks. The default no-credential run records 43 default files plus 4 opt-in provider files, with 407 passed and 15 skipped. The opt-in evidence was executed in isolated provider runs and merged into the report without changing deterministic fixture semantics: the Gemini empirical campaign passed 4/4, the Appendix G companion campaign covered 28 executed and 2 unsupported scenarios, real-embedding semantic gain reached 5/5 targets, and the real dense ablation passed 4/4. A v2 fixed cohort completed real answer and scoring runs for 50 LoCoMo and 50 LongMemEval-S items: LoCoMo completed 49/50 with one empty recall and mean F1 0.0149; LongMemEval-S completed 50/50 with 9/50 judged correct. The records are published in [the fixed-cohort result index](docs/benchmark-results/riko-memory-v2-fixed-cohort-2026-09-22/result-index.json). These results keep REQ-EVAL-035 open because non-empty recall was not reliably relevant and do not upgrade any capability to verified.

The current implementation is a governed Phase 1–5 substrate, not a claim that every production evaluation gate is complete. The package suite covers the implemented state transitions and runs the Appendix F corpus and the Appendix G aggregation; four answer-side corpus metrics remain explicitly unsupported without an answer generator, while focused restart, purge-interruption, load and keyless chaos probes exist. Production benchmark, coverage and production-scale load/chaos evidence must still be collected before treating additional opt-in capabilities as production-ready.

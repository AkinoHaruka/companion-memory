...
The read path never writes candidates, pages, Resident content or authority metadata. Recall results carry source references such as `session:<id>/event:<seq>`. A suppression cue is retained as policy evidence, drives suppression, is omitted from `rawEvidenceCandidates` while active and is exposed only through the guidance-only policy channel; restore and purge remove its active references.

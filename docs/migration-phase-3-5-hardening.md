...
Disable `recallObservationEnabled`, `recallGraphEnabled` and `purgeEnabled` independently. The storage-domain reader remains compatible with v1–v4 records; vector state is restored from durable `index_meta`/`vectors` records and invalidated or rebuilt on mismatch, while graph, alias, projection and conflict state is rebuilt from canonical tables. Validation covers observation anchoring/self-reinforcement rejection, gate decisions, graph multi-hop recall, vector degradation, projection/conflict persistence and the purge journal/recovery behavior; the Appendix F corpus runner and Appendix G aggregate metrics run in the package suite. Focused load, restart, purge-interruption and keyless chaos probes now exist, but production-scale load/chaos evaluation remains an open release gate.

No known rollback security issue is currently recorded for this package. If a compatibility security issue is discovered, the rollback window is limited to one package release, the issue and disablement date must be recorded here, and the new behavior remains the release target.

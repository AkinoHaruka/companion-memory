...
- Current recorded package suite: `docs/spec-execution-report.json` records `45` spec files, `333` passed tests and `11` skipped tests; the opt-in empirical decision campaign passed 4/4 with the Gemini provider.
...
LongMemEval and LoCoMo remain conditional external campaigns; the official LoCoMo checkout exposes 10 conversations and 1,986 QA items, while the official LongMemEval-V2 small-tier source requires a Python 3.11 runner and a published 1,195,604,539-byte trajectories file; no environment-backed run is claimed by this package because the current Gemini text endpoint alone does not provide the required benchmark adapter, prepared data and judge/reader/controller services.
...
- LongMemEval and LoCoMo were not run: the official LoCoMo corpus is available but its 1,986-question Gemini evaluator still needs an adapter to this package, while LongMemEval-V2 requires the Python 3.11 runner plus its 1,195,604,539-byte trajectories source and multiple OpenAI-compatible roles; the conditional benchmark requirement remains open.

...
```
crates/kernel/         Rust。纯决策内核 —— 这是唯一的实现
...
**为什么是 Rust：** 内核的职责收敛成了"纯规则、编译期 schema、无 I/O 阻塞、需穷举测试"，这正是 Rust 的甜点区。而宿主侧（抽取、叙事、整合、DSH 接线）必须留在 TS，因为那里需要 LLM 与会话上下文。

**关于那个被删掉的 TS 原型：** 同一套领域与规则先有一版 TS 实现，67 个测试全绿，之后被删除。原因值得记下来，因为"保留原型作参照"通常是好建议——但它在**自己测试覆盖不到的地方行为是错的**：`Date` 谓词接受任意字符串，于是"散文永不覆盖已解析的确定时刻"这条不变量**实际并未生效**。一个在你在意的那条不变量上与实现相左的参照，比没有参照更糟。

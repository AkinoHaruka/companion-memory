...
```
packages/kernel/       纯决策内核。无 I/O、无 LLM、无宿主依赖、无 Date.now()
                       时间一律由调用方注入 → 将来整体换 Rust，接口不变
...
`packages/kernel` 的边界就是为这件事划的。

**适合 Rust 的部分**（纯规则、编译期 schema、无 I/O 阻塞、需穷举测试）：
- predicate 注册表 → `static` 表
- cardinality 分派、supersede 判定、类型兼容检查
- candidate_score、mention 闸门
- evidence 闭包与 suppression 集
- salience 衰减

**留在 TS 的部分**：抽取、叙事、夜间整合、RuntimeState 更新、DSH 接线——全是宿主职责，且都需要 LLM 与会话上下文。

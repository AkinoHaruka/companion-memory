...
- 外部 benchmark 尚未闭环：REQ-EVAL-035 需要真实、可断点续跑的 LoCoMo 和 LongMemEval campaign，并保留各自的指标产物。首轮 recall-only smoke 已证明两套数据都不是系统性空召回，但完整 answer/scorer campaign 尚未完成；产生空 recall context 或零分的适配器实验不计为证据。

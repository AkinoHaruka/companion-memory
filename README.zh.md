...
Observation 管理、图谱扩展和 raw Session purge 分别由 `recallObservationEnabled`、`recallGraphEnabled`、`purgeEnabled` 控制，默认全部关闭。启用的 Dream reflection 可以创建带 anchor 的 Observation candidate；activation、invalidation 和 suppression 仍是需要认证的管理操作。Observation 必须有 raw/confirmed evidence anchor，始终与 Wiki fact 分开；activation 不会确认 fact，也不会授予 explicit mention permission。普通 `memory_forget` 仍会保留 raw Session 证据。

当前的 `silent_use` renderer 可以消费传入的 `SafeUsageProjection`，且不会披露 raw body 或 source，但还没有接入 projection 的后台生成、持久缓存和 query-time 读取路径；V3.1 禁止 per-turn sanitizing model call。
...
配置 owner namespace 和稳定 Agent preset。两者缺一时长期记忆读写都会 fail-closed。下表完整列出 37 个 live `Config` 字段。`configResponse()` 只返回安全的运行状态；`ownerNamespace`、`apiToken` 和 `apiTokens` 因为会暴露 scope 或凭据而特意不返回，`maxSessionChars` 是内部边界，也不通过该响应返回。OpenAI-compatible embedding 必须配置非空 model、HTTPS endpoint 和 credential reference；deterministic embedding 使用配置的 dimension。OpenRouter 临时验收只把变量注入当前进程：
...
| `embeddingEndpoint` | `''` | OpenAI-compatible embedding provider 使用的 HTTPS endpoint。 | 返回。 |
...
L0 evidence 本身也带一层使用许可；打开 `evidenceClassificationEnabled` 后，它在捕获时而不是读取时确定。规则判定为 `normal` 的一轮对话可以召回，但并不等于发布：只有当 query 显式回忆且主题命中的时候，原文才进入模型上下文，因此没人问起的对话依旧不会被注入；未被分类的事件按 `sensitive` 读取，因为没有任何路径为它做过决定。捕获规则写入的值在能力关闭时是挂起而不是删除，重新打开并重启后会从同一条记录恢复。状态、权威矩阵与回滚语义以 [docs/v3.1-decision-semantics.md](docs/v3.1-decision-semantics.md) 为准。
...
- Observation candidate 可以由启用的 Dream reflection 产生；activation 仍仅限 admin/acceptance HTTP 操作，并不等于 fact confirmation 或 explicit mention permission。
...
- Observation candidate 可以来自启用的 Dream reflection；activation 仍限于需要认证的管理路由，也不等于 canonical confirmation 或 explicit mention permission。

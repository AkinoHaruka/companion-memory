...
- 外部 benchmark 尚未闭环：REQ-EVAL-035 现在已有发布的 v2 固定 cohort 及可断点续跑的 LoCoMo、LongMemEval answer/scorer 产物，但测量结果还不是能力通过。LoCoMo 完成 49/50、平均 F1 为 0.0149；LongMemEval-S 正确 9/50；另有 1 个 LoCoMo 题目空召回。[发布的结果索引](docs/benchmark-results/riko-memory-v2-fixed-cohort-2026-09-22/result-index.json)是审阅入口。非空召回不单独视为相关召回，也不作 verified 能力声明。

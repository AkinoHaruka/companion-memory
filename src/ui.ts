...
<body>
  ...
  <script>
    (() => {
      ...
      const candidateTitle = page => { const title = String(page.title || '').replace(/\s+/g, ' ').trim(); const latinRatio = (title.match(/[A-Za-z]/g) || []).length / Math.max(1, title.length); if (title.length >= 2 && title.length <= 24 && !/[。！？!?；;]/.test(title) && latinRatio < .45) return title; const signal = (title + ' ' + (page.description || page.body || '')).toLowerCase(); if (page.type === 'source') return '会话摘要候选'; if (page.type === 'entity') return /项目|工具|project|tool/.test(signal) ? '长期项目候选' : '用户事实候选'; if (page.type === 'concept') return /边界|不要|安静|quiet|boundary|22:00|十点/.test(signal) ? '沟通边界候选' : '长期偏好候选'; if (page.type === 'episode') return '重要经历候选'; if (page.type === 'emotion') return '近期状态候选'; if (page.type === 'relationship') return '陪伴关系候选'; return '长期记忆候选'; };
      const renderCandidates = records => { const target = $('candidates'); target.replaceChildren(); $('candidate-count').textContent = records.length + ' 条'; if (!records.length) { target.append(node('div', 'panel panel-pad empty', '暂无待确认候选。新的候选会在会话结束后的 Dream 整理中出现。')); return; } for (const record of records) { const page = record.page || record; const row = node('article', 'candidate'); row.append(node('h3', null, candidateTitle(page)), node('p', null, page.description || page.body || '暂无摘要'), node('small', null, typeLabel(page.type) + ' · ' + Math.round((page.confidence || 0) * 100) + '% · ' + (record.sourceConversations || page.sources || []).join(', ') + (record.conflictPageId ? ' · 与现有页面冲突' : ''))); const actions = node('div', 'candidate-actions'); actions.append(actionButton('确认并锁定', 'mint', () => request('/wiki/candidates/' + encodeURIComponent(record.id) + '/confirm', { method: 'POST' })), actionButton('拒绝', 'danger', () => request('/wiki/candidates/' + encodeURIComponent(record.id) + '/reject', { method: 'POST' }))); row.append(actions); target.append(row); } };
      ...
      $('refresh').addEventListener('click', () => load().then(() => showNotice('数据已刷新')).catch(showError)); $('save-token').addEventListener('click', () => { sessionStorage.setItem(tokenKey, $('token-input').value.trim()); load().then(() => showNotice('Token 已应用')).catch(showError); }); $('open-chat').addEventListener('click', () => { location.href = '/'; }); $('dream-now').addEventListener('click', event => runDream(event.currentTarget)); $('status-dream').addEventListener('click', event => runDream(event.currentTarget)); $('generate-demo').addEventListener('click', () => showError('验收 Demo 默认关闭，请使用独立验收脚本。'));
      ...
      const runDemo = async button => { const label = button.textContent; button.disabled = true; button.textContent = '双 Agent 整理中…'; showNotice('已开始生成 200 条对话，完成后会自动刷新页面'); try { await request('/demo/run', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ turns: 100, topic: '用户希望建立一个长期、可靠、尊重边界的个人陪伴助手' }) }); await load(); showNotice('双 Agent 已生成 200 条对话，并完成 Dream 整理'); } catch (error) { showError(error); } finally { button.disabled = false; button.textContent = label || '双 Agent 生成 200 条'; } };

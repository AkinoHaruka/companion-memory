...
export class RikoMemoryService extends Service {
  ...
  private async handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (req.method === 'POST' && parts[0] === 'observations' && parts[1] && parts[2] === 'evidence' && parts.length === 3) {
        const body = await readJsonBody(req)
        if (!body || typeof body !== 'object') throw new Error('observation evidence body must be an object')
        const input = body as Record<string, unknown>
        const supportingRefs = Array.isArray(input.supportingRefs) ? input.supportingRefs.filter((ref): ref is string => typeof ref === 'string') : []
        const contradictingRefs = Array.isArray(input.contradictingRefs) ? input.contradictingRefs.filter((ref): ref is string => typeof ref === 'string') : []
        const observation = await store.updateObservationEvidence(parts[1], {
          supportingRefs, contradictingRefs, ...(typeof input.evidenceAt === 'string' ? { evidenceAt: input.evidenceAt } : {}),
        })
        sendJson(res, observation === undefined ? 404 : 200, observation ?? { error: 'observation not found' }); return
      }

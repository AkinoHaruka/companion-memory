...
import type { MemoryCategory, MemoryItem, MemoryKind } from './types.ts'
...
export interface WikiCandidate {
  ...
  readonly status: 'candidate' | 'rejected' | 'accepted'
...
export class WikiIndex {
  ...
  listPages(options: { status?: WikiPageStatus; type?: WikiPageType } = {}): Array<Pick<WikiPage, 'id' | 'path' | 'title' | 'type' | 'description' | 'status' | 'consent' | 'validUntil' | 'locked' | 'confidence' | 'version' | 'updatedAt'>> {
    return this.pages
      ...
      .map(page => ({
        ...
        status: page.status, consent: page.consent, ...(page.validUntil === undefined ? {} : { validUntil: page.validUntil }),
  ...
  graph(rootPageId?: string, maxHop = 1): { nodes: WikiGraphNode[]; edges: WikiEdge[] } {
    ...
    for (let hop = 0; hop < Math.max(0, Math.min(maxHop, 8)) && frontier.length > 0; hop += 1) {
      const next: string[] = []
...
export function renderWikiMarkdown(page: Omit<WikiPage, 'id' | 'version' | 'updatedAt'> & Partial<Pick<WikiPage, 'version' | 'updatedAt'>>): string {
  const lines = ['---', `type: ${page.type}`, `title: ${yamlScalar(page.title)}`, `description: ${yamlScalar(page.description)}`, 'sources:', ...page.sources.map(source => `  - ${yamlScalar(source)}`), 'tags:', ...page.tags.map(tag => `  - ${yamlScalar(tag)}`), `timestamp: ${yamlScalar(page.timestamp)}`, `confidence: ${page.confidence.toFixed(3)}`, ...(page.sensitivity === undefined ? [] : [`sensitivity: ${page.sensitivity}`]), `status: ${page.status}`, `consent: ${String(page.consent)}`, ...(page.validUntil === undefined ? [] : [`valid_until: ${yamlScalar(page.validUntil)}`]), `locked: ${String(page.locked)}`, `version: ${String(page.version ?? 1)}`, `updated_at: ${yamlScalar(page.updatedAt ?? page.timestamp)}`, ...(page.category === undefined ? [] : [`category: ${page.category}`]), ...(page.kind === undefined ? [] : [`kind: ${page.kind}`]), '---', '', page.body.trim(), '']
  return lines.join('\n')
}
...
export function pageFromMemory(item: MemoryItem, now = new Date().toISOString()): WikiPage {
  ...
  return { id: wikiPageId(path), path, type, title, description: item.content, body: `# 记忆\n\n${item.content}\n\n来源会话：${item.sourceConversations.map(value => `[[${value}]]`).join('、')}`, sources: [...item.sourceConversations], tags: [item.category, item.kind], timestamp: item.observedAt, confidence: item.confidence, sensitivity: item.sensitivity, status: item.status === 'superseded' ? 'superseded' : item.status, consent: item.consent, ...(item.validUntil === undefined ? {} : { validUntil: item.validUntil }), locked: item.status === 'confirmed', version: 1, updatedAt: now, category: item.category, kind: item.kind }
}
...
export function memoryFromPage(page: WikiPage): MemoryItem | undefined {
  ...
  return { id: contentHash(`${page.category}\n${content.trim()}`).slice(0, 24), kind: page.kind, category: page.category, content, confidence: page.confidence, status: page.status, sourceConversations: [...page.sources], observedAt: page.timestamp, ...(page.validUntil === undefined ? {} : { validUntil: page.validUntil }), sensitivity: page.sensitivity ?? 'normal', consent: page.consent }
}
...
function normalizeSensitivity(value: unknown): 'normal' | 'sensitive' | undefined { return value === 'normal' || value === 'sensitive' ? value : undefined }

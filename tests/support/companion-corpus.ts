...
export const companionCorpus: readonly CompanionScenario[] = [
  ...
  { id: 'F.21', category: 'Reranker Failure', setup: { kind: 'unsupported', text: 'RRF fallback' }, userTurn: '你还记得 RRF target 吗？', expected: { label: 'correct injection' }, unsupported: 'RikoMemoryService.Config has no MemoryReranker provider field, and its live HTTP routes do not accept a reranker fault.' },
  { id: 'F.22', category: 'Graph Failure', setup: { kind: 'unsupported', text: 'graph fallback' }, userTurn: '你还记得 graph target 后来怎样？', expected: { label: 'correct injection' }, unsupported: 'RikoMemoryService.Config has no graph/index fault field, and its live HTTP routes do not replace or fault the internal graph index.' },
  ...
  { id: 'F.30', category: 'Deletion Crash', setup: { kind: 'purge-crash-recovery', text: 'interrupted purge' }, userTurn: '你还记得 interrupted purge 吗？', expected: { label: 'correct silence' } },
]
